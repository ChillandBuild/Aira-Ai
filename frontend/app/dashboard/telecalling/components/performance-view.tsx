    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-surface rounded-card p-6 shadow-card ring-1 ring-[#c4c7c7]/15">
        <div className="flex items-center justify-between mb-5 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-200">
            ...
            <input type="date" value={exportSince} onChange={(e) => setExportSince(e.target.value)} className="px-2 py-1 rounded bg-white border border-slate-200 font-body text-xs text-slate-800 focus:outline-none" />
            <span className="text-slate-400 text-xs">to</span>
            <input type="date" value={exportUntil} onChange={(e) => setExportUntil(e.target.value)} className="px-2 py-1 rounded bg-white border border-slate-200 font-body text-xs text-slate-800 focus:outline-none" />
            <button
                ...
                getSortedPerformers().map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-slate-600 font-semibold">{row.connect_rate ? `${Math.round(row.connect_rate * 100)}%` : "0%"}</td>
                      <td className="py-3.5 px-4 text-slate-600 font-medium">{row.avg_talk_seconds ? `${Math.floor(row.avg_talk_seconds / 60)}m ${row.avg_talk_seconds % 60}s` : "0s"}</td>
                      <td className="py-3.5 px-4 text-slate-650 font-medium">{row.idle_minutes_today ? `${Math.round(row.idle_minutes_today)} min` : "0 min"}</td>
                      <td className="py-3.5 px-4 text-slate-800 font-bold text-sm">{row.quality_avg ? `${row.quality_avg.toFixed(1)}/10` : "—"}</td>
      ...
      {selectedCallerId && (
        <div className="space-y-8">
          ...
          <ShiftTimeline callers={callersList} initialCallerId={selectedCallerId} />
        </div>
      )}
