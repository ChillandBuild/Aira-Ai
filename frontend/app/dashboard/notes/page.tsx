...
export default function NotesPage() {
  ...
  const [allNotes, setAllNotes] = useState<NoteWithLead[]>([]);
  ...
  useEffect(() => {
    if (pageMode !== "all_notes") return;
    setAllNotesLoading(true);
    api.notes.all()
      .then((res) => setAllNotes(res.data || []))
      .catch(() => setAllNotes([]))
      .finally(() => setAllNotesLoading(false));
  }, [pageMode]);
  ...
  async function saveEdit(noteId: string) {
    ...
    try {
      ...
      setEditingId(null);
      await refreshNotes();
      if (pageMode === "all_notes") await refreshAllNotes();
    } catch (err) {
