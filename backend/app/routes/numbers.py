...
@router.post("/")
async def create_phone_number(payload: CreatePhoneNumber):
    db = get_supabase()
    insert_data = {
...
async def update_phone_number(number_id: UUID, payload: UpdatePhoneNumber):
    db = get_supabase()
    updates = {}
    ...
    result = db.table("phone_numbers").update(updates).eq("id", str(number_id)).execute()
    if not result.data:
...
async def delete_phone_number(number_id: UUID):
    db = get_supabase()
    active_result = (
        db.table("phone_numbers")
        .select("id")
    ...
    db.table("phone_numbers").update({"status": "archived", "role": "archived"}).eq("id", str(number_id)).execute()
    return {"deleted": True}
