...
async def send_text_message(
    ...
    if track_links and tenant_id and text and "http" in text:
        try:
            from app.services.link_shortener import rewrite_urls_in_text
            text = rewrite_urls_in_text(text, tenant_id=tenant_id, lead_id=lead_id, campaign="reply")
        except Exception as e:
            logger.warning("Link rewrite skipped: %s", e)
