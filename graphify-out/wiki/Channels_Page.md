# Channels Page

> 21 nodes · cohesion 0.10

## Key Concepts

- **page.tsx** (24 connections) — `frontend/app/dashboard/channels/page.tsx`
- **str** (3 connections) — `backend/app/config_dynamic.py`
- **saveSettings()** (3 connections) — `frontend/app/dashboard/channels/page.tsx`
- **invalidate_cache()** (2 connections) — `backend/app/config_dynamic.py`
- **fetchSettings()** (2 connections) — `frontend/app/dashboard/channels/page.tsx`
- **ChannelsHubPage()** (2 connections) — `frontend/app/dashboard/channels/page.tsx`
- **InstagramIcon()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **FacebookIcon()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **Setting** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **SettingsMap** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **FieldDef** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **ActivateResult** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **ChannelHealth** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **TokenAlert** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **SaveState** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **CHANNELS** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **CopyButton()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **OutlinedField()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **SecretField()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **timeAgo()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`
- **WebhookConfigGuide()** (1 connections) — `frontend/app/dashboard/channels/page.tsx`

## Relationships

- [[Upload / Broadcast Page]] (3 shared connections)
- [[Settings Page]] (3 shared connections)
- [[App Settings & Telecalling Config]] (2 shared connections)
- [[Template Submission (Meta)]] (1 shared connections)
- [[Leads Page & API Client]] (1 shared connections)
- [[Telecalling Context & Notes]] (1 shared connections)

## Source Files

- `backend/app/config_dynamic.py`
- `frontend/app/dashboard/channels/page.tsx`

## Audit Trail

- EXTRACTED: 50 (98%)
- INFERRED: 1 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*